...
def calc_stat_weight(folded_atoms, partitionId, partitionStates, partitionSchemes, stateNum, ASA_U_Apolar_unit,
...
def sampleMicrostate(sampleNum):
    ...
    return partitionId, partition, Fraction_folded, Sconf, delASA_ap, delASA_pol, stateFlag, count, end_time - start_time
...
if __name__ == '__main__':
    ...
    sampleSize = int(input("Enter the number of states you want to sample: "))
    ...
    args = (partitionSchemes, partitionStates, df, stack_from_pdb, OTnum, factor, Native_State_dct)



    start_time = time.time()
    with Pool(5, initializer= init, initargs=(args,)) as pool:
        result = pool.map(sampleMicrostate, range(sampleSize))
